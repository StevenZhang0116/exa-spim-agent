...
## Current criteria (Generation 5 — gen-4 guards + single-point-of-approach guard)
...
5. **Single-point-of-approach guard (NEW in gen 5).** A true severed
   continuation touches its partner at ONE localized place — the tip gap.
   Parallel / fasciculating neurites (or one wrapping the other) run close at
   SEVERAL separated places. For each candidate that passed (2)+(3), gather all
   partner nodes within `CONTACT_RADIUS_UM` (5 µm) of any node of the tip's
   fragment; reject the merge if any such contact lies farther than
   `CONTACT_SPREAD_MAX_UM` (6 µm) from the tip contact (a second, separate
   approach region exists). This catches exactly-two-label parallel-adjacency
   merges that guards (2)-(4) cannot see because they only inspect the single tip.

The size gate bounds the COST of a single bad merge; the margin test reduces its
PROBABILITY at crossings; the single-point-of-approach guard rejects parallel
neurites that hug at multiple places; the transitive-fusion guard caps the SPAN
of any merged class so no chain can grow into a long-path over-merge. All required.
...
- **Gen 5 (this revision):** gen-4's (C) guard worked — the 3-label chain
  `375149638+746076720+746076722` is gone and edits dropped 769 -> 727 — but
  Edge Accuracy and the large ERL losses were essentially UNCHANGED (N016 dERL
  -13934, N020 -8859, N010 -2059). The residual damage comes from exactly-TWO-
  label merges that (C) structurally cannot catch. The report flags
  `553856478+744503863` at FIVE well-separated world locations across N013/N016
  (plus `673589912+673589916`, `659384254+659601729`, `375815390+391547478`).
  Two fragments that run close at multiple separated points are PARALLEL /
  fasciculating neurites (or one wrapping the other), not a single severed
  process; fusing them merges two distinct cells and tanks run-length-weighted
  ERL. Root cause in code: guards (A)/(B) only inspect the SINGLE tip — the
  Lowe-ratio test confirms the partner is the tip's nearest fragment, which it
  genuinely is when the two neurites hug — so the multi-contact signature is
  never measured. Fix: keep (A)/(B)/(C) intact and insert a SINGLE-POINT-OF-
  APPROACH guard (B2) for survivors of (A)+(B): gather every partner node within
  `CONTACT_RADIUS_UM` (5 µm) of any node of the tip's fragment and reject the
  merge if any contact lies more than `CONTACT_SPREAD_MAX_UM` (6 µm) from the tip
  contact, since that means a second separate approach region exists. A genuine
  one-gap continuation contacts only near the tip and passes; the flagged
  parallel pairs contact at several places and are rejected. The test reuses the
  existing KD-tree and runs only on post-(A)/(B) survivors, so it stays cheap.
