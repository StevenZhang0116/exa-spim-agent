...
# --- Gen 6 tunables: conservative split-repair to recover OMITTED edges --------
# Diagnosis (gen05 report): fitness 85.4327, %Merged 9.0921, %Split 0.1724 =>
# implied %Omit ~= 5.30, the second-largest error component and untouched by gens
# 1-4. split_label is a dead end here ("no raw label spans >=2 GT neurons in this
# split" — every speculative split trips the over-split watchdog). So the only
# net-positive lever is merge_labels that REATTACHES a dropped fragment across a
# small, clean gap, which converts an omitted (background/dropped) edge into a real
# one. We gate on cheap, robustly-available fields (gap, endpoint degree) so the
# policy actually FIRES, instead of the over-strict gates that fired zero in
# gens 1, 3, 4.
SPLIT_GAP_MAX_UM = 3.0      # only bridge genuinely small, dropped-fragment gaps
SPLIT_MAX_PER_LABEL = 1     # at most one reattachment per source label -> no chaining
SPLIT_NODEB_MAX_DEGREE = 2  # reattach to a tip/shaft endpoint (a real dropped end),
                            #   not into the middle of a busy branch (crossing risk)


def _safe_degree(g, node):
    """Return graph degree of ``node`` or None if unavailable. Crash-safe."""
    try:
...
def propose_edits(sites, ctx) -> list:
    ...
    GEN 6 POLICY — conservative omit-recovery via merge_labels on SplitSites only.
    We deliberately emit NO split_label: the gen05 report states no raw label spans
    >=2 GT neurons here, so any split would only over-split a real neuron and trip
    the watchdog. Instead we reattach dropped fragments across SMALL, CLEAN gaps to
    recover omitted edges (implied %Omit ~= 5.30). Gates use only cheap, always-
    present fields (gap_um, endpoint degree) so the policy actually fires.
    """
    edits = []
    ...
    # Track how many reattachments we've already issued per source label, so a
    # single fragment cannot be chained into many neighbors (mega-label guard).
    per_label_count = {}

    for s in sites:
        kind = getattr(s, "kind", "split")

        # MergeSites: decline. split_label has no GT-confirmable target on this
        # split, and a speculative split only raises %Split (watchdog). Make no
        # change rather than emit a risky edit.
        if kind != "split":
            continue

        # --- SplitSite -> merge_labels (omit-edge recovery) -------------------
        try:
            gap = float(getattr(s, "gap_um", float("nan")))
        except Exception:
            gap = float("nan")
        # Require a genuinely small, well-defined gap (a dropped connection),
        # not a long reach across background (crossing / unrelated-neurite risk).
        if not (gap == gap) or gap > SPLIT_GAP_MAX_UM:  # NaN-safe
            continue

        label_a = getattr(s, "label_a", None)
        label_b = getattr(s, "label_b", None)
        if label_a is None or label_b is None or label_a == label_b:
            continue

        # node_a is always a tip (degree 1). node_b may be tip/shaft/branch; only
        # reattach when node_b is also a clean endpoint (degree <= 2). Reattaching
        # into a high-degree branch point is where unrelated neurites cross, the
        # classic over-merge that manufactures merge errors. If the graph or degree
        # is unavailable we conservatively SKIP rather than guess.
        if g is not None:
            deg_b = _safe_degree(g, getattr(s, "node_b", None))
            if deg_b is None or deg_b > SPLIT_NODEB_MAX_DEGREE:
                continue
        else:
            # No graph to verify endpoint cleanliness: only trust the very smallest
            # gaps, where a dropped-connection interpretation is most likely.
            if gap > (SPLIT_GAP_MAX_UM * 0.5):
                continue

        # Per-label reattachment cap: prevent chaining one fragment into many.
        ca = per_label_count.get(label_a, 0)
        cb = per_label_count.get(label_b, 0)
        if ca >= SPLIT_MAX_PER_LABEL or cb >= SPLIT_MAX_PER_LABEL:
            continue

        try:
            edits.append(s.as_edit())
        except Exception:
            edits.append((label_a, label_b))
        per_label_count[label_a] = ca + 1
        per_label_count[label_b] = cb + 1
